...
class InventoryView(APIView):
  ...
  def get(self, request, id=None, format=None):
    if id is None:
      # TODO serializerが定義されていないのがわからん
      return Response(serializer.data, status.HTTP_400_BAD_REQUEST)
    else:
      # UNIONするためにそれぞれフィールド名を再定義している
      purchase = Purchase.objects.filter(product_id=id).prefetch_related(
        'product'
        ).values(
          "id", "quantity", type=Value('1'), date=F('purchase_date'), unit=F('product__price')
        )
      
      sales = Sales.objects.filter(product_id=id).prefetch_related(
        'product'
      ).values(
  ...
  def post(self, request, format=None):
    ...
    purchase = Purchase.objects.filter(
      product_id=request.data['product']
        ...
        quantity_sum=Coalesce(Sum('quantity'), 0) # 在庫テーブルのレコードを取得
      )
    
    sales = Sales.objects.filter(
      product_id=request.data['product']
      ).aggregate(
        quantity_sum=Coalesce(Sum('quantity'), 0) # 卸しテーブルのレコードを取得
      )
...
class LoginView(APIView):
  ...
  Args:
    APIVew (class): rest_framework.viewsのAPIViewを受け入れる
  """
  # 認証クラスの指定
